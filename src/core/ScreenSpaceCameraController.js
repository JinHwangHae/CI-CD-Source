...
import pickGlobe from "./pickGlobe";
/**
...
function ScreenSpaceCameraController(scene) {
    ...
    this._zoomMousePosition = new Cartesian2();
    this._zoomPosition = new Cartesian3();
    ...
    this.snapOffset = 5; // px

    this.enableVerticalRotating = true;
}
                ...
                if (!defined(centerPosition)) {
                    ...
                    if (Cartesian3.dot(camera.direction, cameraPositionNormal) >= -0.5) {
                        ...
                        const delta = Math.asin(
                            CesiumMath.clamp((remainingDistance / targetDistance) * Math.sin(alpha), -1.0, 1.0)
                        );
...
// original pickGlobe
function originalPickGlobe(controller, mousePosition, result) {
    const scene = controller._scene;
...
function rotate3D(controller, startPosition, movement, constrainedAxis, rotateOnlyVertical, rotateOnlyHorizontal) {
    ...
    if (controller.enableVerticalRotating) {
...
function zoom3D(controller, startPosition, movement) {
    ...
    if (height < controller._minimumPickingTerrainHeight) {
        if (Cartesian2.equals(startPosition, controller._zoomMousePosition)) {
            intersection = Cartesian3.clone(controller._zoomPosition, zoomCVIntersection);
        } else {
            intersection = originalPickGlobe(controller, startPosition, zoomCVIntersection);
            Cartesian2.clone(startPosition, controller._zoomMousePosition);
            Cartesian3.clone(intersection, controller._zoomPosition);
        }
    }
